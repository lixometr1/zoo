<template>
  <div
    class="w-[300px] flex-shrink-0 catalog-modal-menu space-y-1 bg-grey bg-opacity-20"
  >
    <div
    ...
    >
      <div class="flex-y-center">
        <component
          :is="item.icon"
          width="20"
          class="mr-4 text-green catalog-modal-menu__item-icon"
        />
        {{ item.name }}
      </div>
...
.catalog-modal-menu {
  &__item {
    @apply rounded-sm px-5.5 py-[12px] flex-y-center cursor-pointer w-full border-2 border-white font-medium text-grey bg-grey-600;
